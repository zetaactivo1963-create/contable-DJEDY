...
class GoogleSheetsService {
  ...
  async setupSheetHeaders() {
    ...
    for (const [sheetName, headerRow] of Object.entries(headers)) {
      try {
        // Verificar si ya existe header
        const checkResponse = await this.sheets.spreadsheets.values.get({
          spreadsheetId: this.sheetId,
          range: `${sheetName}!A1:A1`,
        });
        
        const hasHeader = checkResponse.data.values && checkResponse.data.values.length > 0;
        
        if (!hasHeader) {
          // Solo crear header si no existe
          ...
          console.log(`✅ Headers creados para: ${sheetName}`);
        }
      } catch (error) {
        console.error(`Error en headers de ${sheetName}:`, error);
      }
    }
...
async registrarDeposito(eventoId, monto, chatId, username) {
  try {
    ...
    if (rowIndex > 0) {
      // 5. Calcular nuevo deposito_inicial (suma acumulada)
      const depositoActual = parseFloat(evento.deposito_inicial) || 0;
      const nuevoDepositoInicial = depositoActual + montoNum;
      
      // Estado SIEMPRE 'en_proceso' para depósitos
      const nuevoEstado = 'en_proceso';
      
      console.log('🔍 DEBUG actualizando Google Sheets:', {
        rowIndex,
        depositoActual,
        nuevoDepositoInicial,
        range: `eventos!E${rowIndex}:H${rowIndex}`,
        valores: [nuevoDepositoInicial, nuevoPagadoTotal, nuevoPendiente, nuevoEstado],
        columnas: ['E=deposito_inicial', 'F=pagado_total', 'G=pendiente', 'H=estado']
      });
      
      // 6. Actualizar las 4 columnas (deposito_inicial, pagado_total, pendiente, estado)
      await this.sheets.spreadsheets.values.update({
        spreadsheetId: this.sheetId,
        range: `eventos!E${rowIndex}:H${rowIndex}`,
  ...
  console.log(`✅ Evento ${eventoId} actualizado: deposito=$${nuevoDepositoInicial}, pagado=$${nuevoPagadoTotal}, pendiente=$${nuevoPendiente}`);
}
...
const reparticion = {
  ...
  empresa: netoParaRepartir * 0.10
      ...
      return {
        ...
        presupuestoTotal: evento.presupuesto_total,
        gastosTotales: gastosTotales,
        netoRepartido: netoParaRepartir,
...
async registrarGastoEvento(eventoId, monto, descripcion, chatId, username) {
  try {
    ...
    // 3. Registrar transacción SOLO para tracking (sin afectar balance)
    await this.registrarTransaccion({
      tipo: 'gasto',
      cuenta: 'EVENTO',  // No va a ninguna cuenta real
      monto: monto,
      descripcion: `[${eventoId}] ${descripcion}`,
      evento_id: eventoId,
      categoria: 'gasto_evento'
    ...
    // NO actualizamos ningún balance
    // El gasto ya salió del presupuesto del cliente, solo se resta del neto
