...
class GoogleSheetsService {
  ...
  async registrarDeposito(eventoId, monto, chatId, username) {
      console.log('🔍 DEBUG registrarDeposito INICIO:', {
    eventoId, 
    monto, 
    montoTipo: typeof monto,
    montoParseado: parseFloat(monto)
  });
    try {
      // Obtener evento
      ...
      // Actualizar pagado_total y pendiente en evento
      // Convertir a números por si acaso
      const pagadoActual = parseFloat(evento.pagado_total) || 0;
      const nuevoPagadoTotal = pagadoActual + parseFloat(monto);
      const nuevoPendiente = evento.presupuesto_total - nuevoPagadoTotal;
...
if (rowIndex > 0) {
  ...
  const nuevoEstado = nuevoPendiente <= 0 ? 'completado' : 'en_proceso';
  
  console.log(`🔍 DEBUG registrarDeposito: Actualizando evento ${eventoId}`);
  console.log(`🔍 - Row ${rowIndex}, F(pagado_total): ${nuevoPagadoTotal}, G(pendiente): ${nuevoPendiente}, H(estado): ${nuevoEstado}`);


    console.log('🔍 DEBUG ANTES de actualizar Google Sheets:', {
    nuevoPagadoTotal,
    nuevoPendiente,
    rowIndex,
  ...
  // Actualizar SOLO las 3 columnas necesarias
  await this.sheets.spreadsheets.values.update({
    spreadsheetId: this.sheetId,
    range: `eventos!F${rowIndex}:H${rowIndex}`,  // F=pagado_total, G=pendiente, H=estado
    valueInputOption: 'USER_ENTERED',  // Cambiar a USER_ENTERED para mejor manejo de números
    resource: { 
      values: [[
        nuevoPagadoTotal.toString(),  // F: pagado_total
        nuevoPendiente.toString(),    // G: pendiente
        nuevoEstado                   // H: estado
      ]] 
    },
  });
...
if (rowIndex > 0) {
  ...
  // Asegurar que monto es número
  const montoNumero = parseFloat(monto) || 0;
  
  console.log(`🔍 DEBUG actualizarBalance: cuenta=${cuentaNombre}, monto=${montoNumero}, esPendiente=${esPendiente}, actual=${balanceActual}, pendiente=${balancePendiente}`);
