...
class GoogleSheetsService {
  ...
  async initialize() {
    try {
      // Obtener credenciales de Base64
      ...
      console.log('📄 Credenciales cargadas para:', credentials.client_email);
...
async addTransaction(transactionData) {
  try {
    const transaction = {
      ...
      event: transactionData.event || transactionData.description || '',
      amount: parseFloat(transactionData.amount),
      timestamp: new Date().toISOString(),
      notes: transactionData.notes || '',
      cuenta: transactionData.cuenta || 'Personal',  // NUEVO
      retencion: parseFloat(transactionData.retencion) || 0,  // NUEVO
      balance_final: parseFloat(transactionData.amount) - (parseFloat(transactionData.retencion) || 0)  // NUEVO
    };

    // Actualizar rango para incluir nuevas columnas
    await this.sheets.spreadsheets.values.append({
      spreadsheetId: this.sheetId,
      range: 'transactions!A:K',  // Cambiado de A:H a A:K
      valueInputOption: 'RAW',
      insertDataOption: 'INSERT_ROWS',
      resource: {
    ...
    console.log(`✅ Transacción registrada: ${transaction.transaction_type} $${transaction.amount} - ${transaction.cuenta}`);
    return transaction;
  } catch (error) {
