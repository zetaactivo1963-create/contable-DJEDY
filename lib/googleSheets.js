...
class GoogleSheetsService {
  ...
  async registrarDeposito(eventoId, monto, chatId, username) {
    try {
      ...
      if (rowIndex > 0) {
        // Actualizar evento - asegurar que son números
        rows[rowIndex - 1][5] = nuevoPagadoTotal.toString(); // pagado_total
        rows[rowIndex - 1][6] = nuevoPendiente.toString(); // pendiente
        
        // Cambiar estado si está pagado completamente
        if (nuevoPendiente <= 0) {
          rows[rowIndex - 1][7] = 'completado';
        }
        ...
        await this.sheets.spreadsheets.values.update({
          ...
          valueInputOption: 'RAW',
          resource: { values: [[nuevoPagadoTotal, nuevoPendiente, rows[rowIndex - 1][7]]] },
        });
      }
